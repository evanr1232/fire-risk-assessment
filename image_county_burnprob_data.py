...
try:
    ...
    if 'county' not in df_images.columns:
        raise ValueError("Image CSV must contain a 'county' column with format 'County, State'.")
...
# Clean image county names: remove ', US', strip whitespace, standardize capitalization
df_images['county_clean'] = (
    df_images['county']
...
# Clean NRI table: combine COUNTY + STATE as join key, standardize capitalization
df_nri['join_key'] = (
    df_nri[NRI_COUNTY_COL].str.strip().str.title() + ', ' +
    df_nri[NRI_STATE_COL].str.strip().str.title()
)

# Select only the join key and wildfire score, drop duplicates
...
df_merged = df_merged.drop(columns=['join_key', 'county_clean'])
